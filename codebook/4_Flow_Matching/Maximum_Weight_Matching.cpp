...
struct WeightGraph { // 1-based
  ...
  vector<int> slk, match, st, pa, sl, vis;
  ...
  void add_blossom(int u, int o, int v) {
    ...
    lab[b] = 0, sl[b] = 0, match[b] = match[o];
    vector<int> f = {o};
    for (int t : {u, v}) {
      reverse(1 + ALL(f));
  ...
  void expand_blossom(int b) {
    ...
    for (int x : split_flo(flo[b], xr)) {
      ...
      pa[xs] = g[x][xs].u, sl[xs] = 1, sl[x] = 0;
      slk[xs] = 0, set_slk(x), q_push(x), xs = -1;
    }
    for (int x : flo[b])
      if (x == xr) sl[x] = 1, pa[x] = pa[b];
      else sl[x] = -1, set_slk(x);
    st[b] = 0;
  }
  bool on_found_edge(const edge &e) {
    if (int u = st[e.u], v = st[e.v]; sl[v] == -1) {
      int nu = st[match[v]];
      pa[v] = e.u;
      sl[v] = 1;
      slk[v] = slk[nu] = sl[nu] = 0;
      q_push(nu);
    } else if (sl[v] == 0) {
