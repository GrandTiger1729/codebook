...
\inputcode{Sanitize Fix}{1_Basic/sanitize_fixer.cpp}
% \subsection{Texas hold'em}
% \lstinputlisting{1_Basic/Texas_holdem.cpp}


\section{Graph}
\inputcode{EBCC*}{2_Graph/EBCC.cpp} % test by caido, Lib-Checker Two-Edge-Connected Components
\inputcode{VBCC*}{2_Graph/VBCC.cpp} % test by caido, Lib-Checker Biconnected Components
\inputcode{SCC*}{2_Graph/SCC.cpp} % test by caido, Lib-Checker Strongly Connected Components
\inputcode{2SAT*}{2_Graph/2SAT.cpp} % needs SCC; test by Lib-Checker Two SAT, local brute force (either/set_value/at_most_one)
...
\inputcode{Minimum Steiner Tree*}{2_Graph/MinimumSteinerTree.cpp} % needs N, T, INF; test by luogu P6192, Lib-Checker Minimum Steiner Tree, local brute force (16000 graphs, with and without vcst)
\inputcode{Dominator Tree*}{2_Graph/Dominator_Tree.cpp} % test by CF 100513 L, Lib-Checker Dominator Tree
\inputcode{Minimum Arborescence*}{2_Graph/Minimum_Arborescence_fast.cpp} % test by luogu P4716, Lib-Checker Directed MST
...
\inputcode{Mergeable Heap}{3_Data_Structure/min_heap.cpp} % test by Lib-Checker Directed MST (through Minimum Arborescence)
\inputcode{BIT kth*}{3_Data_Structure/BIT_kth.cpp} % test by CSES 1076, Lib-Checker Ordered Set
...
\inputcode{Heavy light Decomposition*}{3_Data_Structure/Heavy_light_Decomposition.cpp} % test by CSES Path Queries II, Lib-Checker Vertex Add Path Sum
\inputcode{Centroid Decomposition*}{3_Data_Structure/Centroid_Decomposition.cpp} % test by TIOJ 1171
\inputcode{Treap*}{3_Data_Structure/Treap.cpp} % test by Lib-Checker Ordered Set
...
\inputcode{Link cut tree*}{3_Data_Structure/link_cut_tree.cpp} % test by luogu P3690, Lib-Checker Dynamic Tree Vertex Add Path Sum
...
\inputcode{Bipartite Matching*}{4_Flow_Matching/Bipartite_Matching.cpp} % test by Lib-Checker Matching on Bipartite Graph; cover() by local brute force
\inputcode{Kuhn Munkres*}{4_Flow_Matching/Kuhn_Munkres.cpp} % test by CF 101239 C (World Finals' problem), Lib-Checker Assignment Problem
\inputcode{MincostMaxflow*}{4_Flow_Matching/MincostMaxflow.cpp} % test by Lib-Checker Assignment Problem, local stress vs SPFA reference
...
\inputcode{Suffix Array*}{5_String/SA_LCP_becaido.cpp} % test by caido, Lib-Checker Suffix Array
\inputcode{SAIS C++20*}{5_String/SAIS-C++20.cpp} % test by Lib-Checker Suffix Array
% \inputcode{SAIS*}{5_String/SAIS-old.cpp} % test by CF 104901 H, Lib-Checker Suffix Array
\inputcode{Aho-Corasick Automatan*}{5_String/Aho-Corasick_Automatan.cpp} % test by CF 102511 G (World Finals' problem), Lib-Checker Aho-Corasick
\inputcode{Smallest Rotation*}{5_String/Smallest_Rotation.cpp} % test by CSES Minimal Rotation
\inputcode{De Bruijn sequence*}{5_String/De_Bruijn_sequence.cpp} % test by CF 102001 C
\inputcode{Extended SAM*}{5_String/exSAM.cpp} % test by CF 616 C, Lib-Checker Number of Substrings
\inputcode{PalTree*}{5_String/PalTree.cpp} % test by APIO 2014 palindrome, Lib-Checker Eertree
\inputcode{Main Lorentz}{5_String/MainLorentz.cpp} % test by Lib-Checker Run Enumerate


\section{Math}
\inputcode{ax+by=gcd(only exgcd *)}{6_Math/ax+by=gcd.cpp} % exgcd test by NTUJ 110
\inputcode{Floor and Ceil}{6_Math/floor_ceil.cpp}
\inputcode{Floor Enumeration}{6_Math/floor_enumeration.cpp} % test by Lib-Checker Enumerate Quotients
\inputcode{Mod Min}{6_Math/ModMin.cpp} % test by Lib-Checker Min of Mod of Linear
\inputcode{Gaussian integer gcd}{6_Math/Gaussian_gcd.cpp} % test by Lib-Checker GCD of Gaussian Integers
% \inputcode{floor sum*}{6_Math/floor_sum.cpp} % test by AtCoder Library Practice Contest C, CF 100920 J
\inputcode{Miller Rabin*}{6_Math/Miller_Rabin.cpp} % test by NTUJ 1237, Lib-Checker Primality Test
% \inputcode{Big number{6_Math/Big_number.cpp}}
%\inputcode{Fraction}{6_Math/Fraction.cpp}
\inputcode{System of Linear Equations}{6_Math/Linear_Equations.cpp} % needs MAXN and a field type T; test by Lib-Checker System of Linear Equations (modint), local brute force (fraction)
...
\inputcode{PiCount*}{6_Math/PiCount.cpp} % test by luogu P7884, Lib-Checker Counting Primes
\inputcode{Discrete Log*}{6_Math/DiscreteLog.cpp} % test by Lib-Checker Discrete Logarithm
\inputcode{Berlekamp Massey}{6_Math/Berlekamp-Massey.cpp} % returns c 1-based, feeds Poly::LinearRecursion directly; test by Lib-Checker Find Linear Recurrence, local round trip through LinearRecursion
\subsection{Primes}
...
\section{Polynomial}
\inputcode{Fast Fourier Transform}{7_Polynomial/Fast_Fourier_Transform.cpp} % test by Lib-Checker Convolution mod 1e9+7 (through FFT Mod)
\inputcode{FFT Arbitrary Mod}{7_Polynomial/Fast_Fourier_Transform_Mod.cpp} % test by Lib-Checker Convolution mod 1e9+7
\inputcode{Number Theory Transform*}{7_Polynomial/NTT_stdabs.cpp} % std_abs verbatim; the header block (mod/G/N, sz/all, add/sub/mul/Pow, fac/facp) is what it needs; test by Lib-Checker Convolution
\subsection{NTT Primes}
\input{7_Polynomial/NTTprime.tex}
\inputcode{Fast Walsh Transform*}{7_Polynomial/Fast_Walsh_Transform.cpp} % test by luogu P6097, local brute force (2000 subset convolutions under ASan/UBSan)
\inputcode{Polynomial Operation}{7_Polynomial/Operation_stdabs.cpp} % std_abs verbatim; needs NTT, QuadraticResidue, and build() for TaylorShift/SamplingShift; test by Lib-Checker Inv/Exp/Log/Pow/Sqrt of FPS, Polynomial Taylor Shift, Shift of Sampling Points
\inputcode{Linear Recursion}{7_Polynomial/FastLinearRecursion_stdabs.cpp} % std_abs verbatim; c is 0-based here, BerlekampMassey returns 1-based; TLEs Lib-Checker Kth Term of Linearly Recurrent Sequence at d=1e5 (Divide re-inverts the fixed modulus every round)
...
\inputcode{Heart}{8_Geometry/Heart.cpp} % circenter test by Lib-Checker Minimum Enclosing Circle
\inputcode{point in circle}{8_Geometry/point_in_circle.cpp}
\inputcode{Convex hull*}{8_Geometry/Convex_hull.cpp} % test by Zerojudge b398, Lib-Checker Static Convex Hull
...
\inputcode{Polar Angle Sort*}{8_Geometry/Polar_Angle_Sort.cpp} % test by NTUJ 2270, Lib-Checker Sort Points by Argument
...
\inputcode{Minimum Enclosing Circle*}{8_Geometry/Minimum_Enclosing_Circle.cpp} % test by TIOJ 1093, Lib-Checker Minimum Enclosing Circle
...
\inputcode{Closest Pair}{8_Geometry/ClosestPair.cpp} % test by Lib-Checker Closest Pair
...
\inputcode{DynamicConvexTrick*}{9_Else/DynamicConvexTrick.cpp} % test by TIOJ 1921, Lib-Checker Line Add Get Min
%\inputcode{cyclicLCS}{9_Else/cyclicLCS.cpp}
\inputcode{All LCS*}{9_Else/All_LCS.cpp} % test by Lib-Checker Prefix-Substring LCS
\inputcode{DLX*}{9_Else/DLX.cpp} % test by TIOJ 1333, 1381
\subsection{Matroid Intersection}
\input{9_Else/Matroid.tex}
\inputcode{Matroid Intersection}{9_Else/MatroidIntersection.cpp} % test by local brute force (graphic x colourful)
...
\inputcode{Tree Hash*}{9_Else/tree_hash.cpp} % needs G; test by CSES Tree Isomorphism I, Lib-Checker Rooted Tree Isomorphism Classification
\inputcode{Binary Search On Fraction*}{9_Else/BinarySearchOnFraction.cpp} % test by ABC 333 G after adjusted to int128, Lib-Checker Rational Approximation
\inputcode{1D1D}{9_Else/1d1d.cpp}
\inputcode{SMAWK}{9_Else/smawk.cpp} % test by Lib-Checker Min Plus Convolution (Convex and Arbitrary)
